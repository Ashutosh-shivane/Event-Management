...
export function ManagerAssignedEventPage() {
 ...
 useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setLoading(false);
        console.log(response.data); // done loading
      })
      .catch((err) => {
...
{loading && <Spinner />}
  ...
  {new Date(event.startAt).toLocaleDateString()} at {new Date(event.startAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                ...
                <div className="flex items-center text-sm text-gray-600">
                  ...
                  {/* {event.requiredVolunteer}/{event.requiredVolunteer} attendees */}
                   {event.requiredVolunteer} attendees
                </div>
              </div>
